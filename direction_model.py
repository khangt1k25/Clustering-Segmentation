...
class MaskModelGRU(nn.Module):
    def __init__(
        ...
        self.embeddingw = nn.Linear(in_features= input_size,out_features=embedding_size)
        self.embeddingwplus = nn.Linear(in_features= input_size,out_features=embedding_size)
        ...
        self.mask_head = nn.Sequential(
            nn.Linear(in_features=self.hidden_size*self.D, out_features=1024),
            nn.ReLU(),
            nn.Linear(in_features=1024, out_features=self.input_size),
        )
    ...
    def forward(self, w1: torch.Tensor, w2_plus: torch.Tensor):
        ...
        w1 = self.embeddingw(w1)
        w2_plus = self.embeddingwplus(w2_plus)
...
if __name__ == "__main__":
    model = MaskModelGRU(num_layers=2, input_size=512, bidirectional=True)
    print(model)
    w1 = torch.randn(8, 512)
    w1_plus = torch.rand(1, 14, 512)

    w2_plus = torch.rand(8, 14, 512)
    output = model.forward(w1, w2_plus)
    print(output.shape)